 ...
 * v2.9.0 - Uses centralized Control Board client
 */

import { mergePopups } from './controlboard';

// Static popup configurations - ALL POPUPS FROM FOLDERS
// These serve as fallback for legacy popups
const staticPopups = {
...
/**
 * Get popup configuration by ID
 * Merges static and dynamic popups (dynamic takes precedence)
 * @param {string} popupId - The popup ID
 * @returns {Promise<Object|null>} Popup configuration or null
 */
export async function getPopupConfig(popupId) {
  const allPopups = await mergePopups(staticPopups);
  return allPopups[popupId] || null;
}

/**
 * Get all available popups
 * @returns {Promise<Object>} All popup configurations
 */
export async function getAllPopups() {
  return mergePopups(staticPopups);
}
