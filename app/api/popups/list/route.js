/**
 * List All Popups API
 * Returns all available popups (static + dynamic from Control Board)
 * v2.9.0 - For admin dashboard and split test selection
 */

import { getSecurityHeaders, mergeHeaders } from '@/lib/security-headers';
import { mergePopups } from '@/lib/controlboard';

// Static popup configurations (fallback for legacy popups)
const staticPopups = {
...
export async function GET(req) {
  try {
    ...
    const source = searchParams.get('source') || 'all'; // 'all', 'static', 'dynamic'

    let allPopups = {};

    if (source === 'static') {
      allPopups = staticPopups;
    } else if (source === 'dynamic') {
      const { popups } = await fetchDynamicPopups();
      allPopups = popups;
    } else {
      // Merge static and dynamic
      allPopups = await mergePopups(staticPopups);
    }

    // Format the response
    let response;
    if (format === 'minimal') {
      // Return just id and name for dropdowns
      response = Object.entries(allPopups).map(([id, config]) => ({
        id,
        name: config.name || id,
        tagId: config.tagId
      }));
    ...
    return Response.json({
      success: true,
      count: Object.keys(allPopups).length,
      popups: response,
      source,
      format
    }, {
