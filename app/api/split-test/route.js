 ...
 * v2.9.0 - Dynamic popup loading from Control Board
 */

import { getSecurityHeaders, mergeHeaders } from '@/lib/security-headers';
import { fetchDynamicPopups, fetchSplitTest } from '@/lib/controlboard';

// Static popup configurations - ALL POPUPS FROM FOLDERS
// These serve as fallback for legacy popups not yet in Control Board
const staticPopups = {
...
/**
 * Get all available popups (static + dynamic from Control Board)
 */
async function getAllPopups() {
  ...
  // Merge: dynamic takes precedence over static
...
export async function GET(req) {
  ...
  try {
    ...
    const test = await fetchSplitTest(testId);
    ...
    if (test.status === 'completed' && test.winnerPopupId) {
      ...
      if (!winnerPopup) {
        return Response.json(
          { success: false, error: 'Winner popup not found: ' + test.winnerPopupId },
          { status: 404, headers: mergeHeaders({
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'GET, OPTIONS',
            'Access-Control-Allow-Headers': 'Content-Type'
    ...
    if (!variantData || !variantData.popupId) {
      return Response.json(
        { success: false, error: `Invalid variant ${variant} configuration` },
